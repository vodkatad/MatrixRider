...
All the other values represents tradeoffs between these two extremes. 
...
In the examples of the package you can find a simple R script that calculates a
ffinities for all the Vertebrates
...
object with the desidered counts (need to be integer values)
